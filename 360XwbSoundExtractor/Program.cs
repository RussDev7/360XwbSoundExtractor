...
/// <summary>
/// Minimal XWB sound extractor for standard XACT wave banks.
/// 
/// Supported behavior:
/// - Reads standard XWB banks with either:
///   - "WBND" (little-endian)
///   - "DNBW" (big-endian / Xbox 360 style)
/// - Extracts PCM entries to WAV
/// - Extracts ADPCM entries to WAV
/// - Dumps WMA/XMA entries as raw payloads with metadata sidecars
///
/// Notes:
/// - This is intentionally a simple standalone extractor.
/// - Compact wave banks are not supported yet.
/// - Entry names are only used when the bank actually contains an entry-name segment.
/// - For big-endian PCM banks, 16-bit PCM sample data is byte-swapped before WAV output.
/// </summary>
internal static class Program
{
    ...
    /// This simple extractor does not handle compact banks.
    /// </summary>
    ...
    /// Console entry point.
    /// Usage:
    ///   XwbSoundExtractor.exe &lt;input.xwb&gt; &lt;outputFolder&gt;
    /// </summary>
    private static int Main(string[] args)
    {
        if (args.Length < 2)
        {
            ...
            Console.WriteLine("  XwbSoundExtractor.exe <input.xwb> <outputFolder>");
            Console.WriteLine();
            Console.WriteLine("Example:");
            Console.WriteLine("  XwbSoundExtractor.exe Sounds.xwb ExtractedSounds");
            return 1;
        }

        try
        {
            ExtractXwb(args[0], args[1]);
        ...
        using (BinaryReader br = new BinaryReader(fs, Encoding.ASCII, leaveOpen: true))
        {
            ...
            if (signature == "WBND")
                ...
                throw new InvalidDataException(
                    "Not an XACT wave bank (.xwb). Signature was '" + signature + "'.");
            ...
            Console.WriteLine($"Version={version}, HeaderVersion={headerVersion}");
            for (int i = 0; i < segments.Length; i++)
                Console.WriteLine($"Seg{i}: Off=0x{segments[i].Offset:X8} Len=0x{segments[i].Length:X8}");
            ...
            if ((bank.Flags & FLAG_COMPACT) != 0)
                throw new NotSupportedException("Compact wave banks are not handled by this simple extractor yet.");
                ...
                switch (formatTag)
                {
                    ...
                    case TAG_WMA:
                        WriteRawWithMetadata(
                            ...
                            dataLength,
                            basePath + ".xwma.bin",
                            "WMA/xWMA entry dumped as raw payload by simple extractor.");
                        WriteMetadata(
                            basePath + ".txt",
                            rawName,
                            entry,
                    ...
                    case TAG_XMA:
                        WriteRawWithMetadata(
                            ...
                            dataLength,
                            basePath + ".xma.bin",
                            "XMA entry dumped as raw payload by simple extractor.");
                        WriteMetadata(
                            basePath + ".txt",
                            rawName,
                            entry,
